...
public:
  ...
  // Assign from a float array
  matrix33      &operator =  (const float* m) {
    col[0] = &m[0];
	col[1] = &m[3];
	col[2] = &m[6];
    return *this;
...
public:
  ...
  // Constructor with initializing a float array
  matrix44(const float* m) { *this = m; }
...
public:
  ...
  // Assign from a float array
  matrix44      &operator =  (const float* m) {
    col[0] = &m[0];
	col[1] = &m[4];
	col[2] = &m[8];
	col[3] = &m[12];
