...
//#include "Window.h"														// Header File For The NeHeGL Basecode
...
LRESULT CALLBACK WindowProc (HWND hWnd, UINT uMsg, WPARAM wParam, LPARAM lParam)
{
	...
	if (!TrackMouseEvent())
	{
		// Error Tracking Mouse Event
		MessageBox (HWND_DESKTOP, "Error Tracking Mouse Event", "Error", MB_OK | MB_ICONEXCLAMATION);
	...
	switch (uMsg)														// Evaluate Window Message
	{
		...
		*/
		...
		case WM_KEYDOWN:												// Update Keyboard Buffers For Keys Pressed
			...
			{
				keyboardManager.SetKeyDown(wParam, TRUE);				// Set The Selected Key (wParam) To True
				return 0;												// Return
			}
			break;														// Break
		...
		case WM_KEYUP:													// Update Keyboard Buffers For Keys Released
			if ((wParam >= 0) && (wParam <= 255))						// Is Key (wParam) In A Valid Range?
			{
				keyboardManager.SetKeyDown(wParam, FALSE);					// Set The Selected Key (wParam) To False
				return 0;												// Return
			}
			break;														// Break
		...
		case WM_LBUTTONDOWN:
			if (GetCapture() != hWnd)
				SetCapture(hWnd);
			mouseManager.MouseLDown(lParam);
			return 0;
//			break;

		case WM_LBUTTONUP:
			if (!mouseManager.GetMouseRDown())
				ReleaseCapture();
			mouseManager.MouseLUp(lParam);
			return 0;
//			break;
		...
		case WM_RBUTTONDOWN:
			if (GetCapture() != hWnd)
				SetCapture(hWnd);
			mouseManager.MouseRDown(lParam);
			return 0;
//			break;

		case WM_RBUTTONUP:
			if (!mouseManager.GetMouseLDown())
				ReleaseCapture();
			mouseManager.MouseRUp(lParam);
			return 0;
//			break;
